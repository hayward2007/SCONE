...
## 12. 추가 발견 (2026-09-03): yaw 명령에서 heading 비용이 무한 적분된다

`walk_v3` 작업 중 같은 결함을 v3에서 발견해 고치면서 v2에도 그대로 남아 있음을
확인했다. `walk_learn`은 heading을 **유계 양의 보상** `exp(-e²/0.60²)`으로 썼는데,
v2는 이를 **비용** `-0.25·min(25,(e/0.35)²)`으로 바꾸면서 목표 heading을 yaw
명령으로 적분하는 v1 코드(`_target_heading += command_yaw · dt`)를 그대로 유지했다.
실제 yaw rate가 명령보다 조금이라도 부족하면 heading 오차가 계속 쌓인다.

`walk_v2` 환경에서 residual 0으로 직접 측정한 값이다.

| 명령 | 달성 yaw rate | 2초 후 오차 / 누적 heading 항 | 10초 후 오차 / 누적 heading 항 |
| --- | ---: | ---: | ---: |
...
§11.0의 6.6M-step run의 episode return이 약 42.6이었다. 즉 **yaw 명령 구간에서는
정책이 제거할 수 없는 -30~-40 규모의 항이 episode return 전체와 맞먹는다.**
medium/full curriculum은 yaw를 샘플링하므로 학습 신호의 상당 부분이 이 항에
지배됐다. v2가 회전을 배우지 못한 원인 후보로 이 항을 추가한다.

전진 명령에서는 -0.01로 무해하므로 §5~§11의 전진 관련 분석은 영향을 받지 않는다.

`walk_v3`는 heading을 **직진 유지 전용** 목표로 바꿨다: yaw 명령이 `0.05 rad/s`를
넘으면 목표 heading을 현재 몸체 heading으로 재고정하고 비용도 끈다
([`22`](22-walk-v3-residual-design.md) §3). v2를 다시 살릴 경우에만 같은 gate를
적용한다. 지금 v2를 수정하면 환경 의미가 바뀌어 기존 run과 비교할 수 없게 되므로,
활성 라인이 v3인 동안에는 문서화만 한다. 같은 코드가 세 사본으로 갈라진 구조
문제는 [`24`](24-refactoring-assessment.md) §3.1에 정리했다.
