"""Model-based alternating-tripod gait generation for SCONE.
...
class TripodGait:
    ...
    def _infer_support_points(
        ...
        for leg in range(1, 7):
            ...
            support_patch = world_vertices[
                body_vertices[:, 2]
                <= lowest_height + TripodGait.SUPPORT_PATCH_DEPTH
            ]
    ...
    @property
    def nominal_motor_degrees(self) -> NDArray[np.float64]:
        return self._nominal_motor_degrees.copy()
    ...
    def reset_from_controller(self) -> NDArray[np.float64]:
        ...
        if self.controller is None:
            raise RuntimeError("no controller was supplied to TripodGait")
        raw = np.array(
...
# Backward-compatible names for existing imports. ``TripodGait`` is the
# canonical SCONE API and ``non_rl`` remains only as a serialized CLI alias.
NonRLWalk = TripodGait
PhoenixTripodGait = TripodGait
